...
class Env():
    def __init__(self):
        ...
        self.pub_safe_vel   = rospy.Publisher('/turtlebot2i/safety/vel_scale', VelocityScale, queue_size=1) #init publisher
        ...
        self.n_direction          = 12
        self.mid_index            = self.n_direction/2 + 0.5
    ...
    def getState(self, safety_risk_msg):
        ...
        return obstacle_distances + [min(obstacle_distances), np.argmin(obstacle_distances)-self.mid_index, self.robot_linear_speed, self.robot_angular_speed, self.risk_max, self.r_warning, self.r_clear], done

    def getEmptyState(self):
        return list(np.ones((self.n_direction))*self.camera_far_clipping) + [0.0, 0.5, 0.0, 0.0, 0.0, 0.31, 0.32]
    ...
    def setReward(self, state, done, action):
        nearest_obstacle_distance  = state[-7]
        nearest_obstacle_direction = state[-6] #index -5,5 start from right side of the robot
        risk_max = state[-3]

        yaw_reward = 1.0
        if (nearest_obstacle_direction <= -self.n_direction/6):#obstacle is on the right 
            if (action >= 10):                    #robot turns right
                yaw_reward = -(action-9)/6
        elif (nearest_obstacle_direction >= self.n_direction/6):#obstacle is on the left
