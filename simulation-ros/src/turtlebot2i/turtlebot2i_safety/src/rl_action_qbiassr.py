...
DEFAULT_TEMPERATURE = var.TEMPERATURE
...
def setup():
    ...
    threshold = 0.01
    size_sequence = int(math.log(threshold) / math.log(var.GAMMA * var.LAMBDA))

    # size_sequence limits: [4, n_states/4]
    lower_limit = 4
    upper_limit = int(var.n_states / 4)
    ...
    n_inputs = var.n_inputs
    n_states = var.n_states
    comb = np.array(
