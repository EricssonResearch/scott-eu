...
def setup_task():
    ...
    var.in_sizes = in_sizes
    var.n_states = n_states
    var.in_data = input_data

    var.n_outputs = n_outputs
    var.out_values = out_values
    var.out_names = out_names
    var.out_sizes = out_sizes
    var.n_actions = n_actions
    var.out_data = output_data



    print("Task {} \t {} states \t {} actions".format(var.NAME, n_states,
                                                      n_actions))
...
def obtain_reward(s, a, sp):
    ...
    r = training_rl.get_reward()  # (s,a, sp) arguments not needed here
    return r
