...
class ReinforceAgent():
    def __init__(self, env, training_mode):
        ...
        if self.load_model:
            ...
            #self.model.load_weights(self.dirPath + str(self.load_episode) + ".h5", by_name=True)
    ...
    def buildModel(self):
        ...
        #cnn.summary()
        #cnn.add(Dense(64, activation='relu', kernel_initializer='lecun_uniform'))
        
        combinedInput = concatenate([cnn.output, mlp.output])
 
        #x = Dense(16, activation='relu', kernel_initializer='lecun_uniform')(combinedInput)
    ...
    def trainModel(self, target=False):
        ...
        with self.graph.as_default():
            for i in range(self.batch_size):
                ...
                #q_value = self.model.predict([states[1], states[0]])
    ...
    def training_callback(self, data):
        if len(self.distance_stack) < self.stack_len:
            ...
            #self.env.publishScaleSpeed(1.0, 0.0)
            if len(self.distance_stack) == self.stack_len:
