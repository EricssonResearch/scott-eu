...
IMAGE_DIR = os.path.join(ROOT_DIR, "image_test3")
...
count_test = os.listdir(IMAGE_DIR)
for i in range(0, len(count_test)):
    path = os.path.join(IMAGE_DIR, count_test[i])
    if os.path.isfile(path):
        file_name = next(os.walk(IMAGE_DIR))[2]
        image = skimage.io.imread(os.path.join(IMAGE_DIR, count_test[i]))
        ...
        r = results[0]
        visualize.display_instances(count_test[i], image, r['rois'], r['masks'], r['class_ids'], 
