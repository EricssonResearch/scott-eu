...
class visual_genome(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if len(_annotation['regions']) == 0:
            print("zero: ",index,self.annotations[index]['path'],_annotation)
        ...
        #print ('gt_boxes shape : ',index,_annotation['regions'],gt_boxes_region.shape)

        if len(_annotation['regions']) != 0:
            #print("Normal: ", len(_annotation['regions']))
            gt_boxes_region[:, 0:4] = torch.FloatTensor([reg['box'] for reg in _annotation['regions']]) * im_scale
        #print ('gt_boxes: ', index ,torch.FloatTensor([reg['box'] for reg in _annotation['regions']]) * im_scale)
            ...
            print("ALERTTTTT: ",index,self.annotations[index]['path'], len(_annotation['regions']))
            #print("zero: ", _annotation['objects'])
