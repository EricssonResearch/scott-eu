...
def bbox_transform_inv_hdn(boxes, deltas):
    ...
    #print ('Before conversion Boxes type: ',boxes.dtype, boxes.shape)
    #print ('Deltas type: ',deltas.dtype)
    #boxes = np.float32(boxes)
    boxes = np.array(boxes).astype(deltas.dtype, copy=False)
    #temp_boxes = np.empty(shape= boxes.shape, dtype= deltas.dtype)
    #print ('After conversion Boxes Type: ', boxes.dtype, boxes.shape)
