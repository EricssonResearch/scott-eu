...
class HDN_base(nn.Module):
    ...
    def build_loss_object(self, cls_score, bbox_pred, roi_data):
        ...
        ce_weights[0] = float(fg_cnt) / float(bg_cnt + 1e-5)
        ...
        loss_box = F.smooth_l1_loss(bbox_pred, bbox_targets, size_average=False) / (fg_cnt.type(torch.cuda.FloatTensor) + 1e-5)
        # print loss_box
