...
class Hierarchical_Descriptive_Model(HDN_base):
    ...
    def interpret_result(self, cls_prob, bbox_pred, rois, cls_prob_predicate, 
                        mat_phrase, im_info, im_shape, nms=True, clip=True, min_score=0.5, 
                        top_N = 50, use_gt_boxes=False):
        scores, inds = cls_prob[:, 1:].data.max(1)
        inds += 1
        scores, inds = scores.cpu().numpy(), inds.cpu().numpy()
        predicate_scores, predicate_inds = cls_prob_predicate[:, 1:].data.max(1)
        ...
        # nms
        if nms and pred_boxes.shape[0] > 0:
            pred_boxes, scores, inds, keep_keep = nms_detections(pred_boxes, scores, 0.25, inds=inds)
            keep = keep[keep_keep]
        ...
        #return pred_boxes, scores, inds, subject_inds, object_inds, predicate_inds, region_list, predicate_scores, subject_scores, object_scores, relationship_scores
    ...
    def describe(self, im_data, im_info, top_N=[50]):
        ...
        #     [[im_data.shape[1], im_data.shape[2], im_scales[0]]],
        #     dtype=np.float32)
        image_shape = np.array([im_data.shape[2], im_data.shape[3], im_data.shape[1]])
        ...
        # print 'im_scales[0]', im_scales[0]
        return region_caption.numpy(), region_list, region_pred_boxes, logprobs.numpy(), class_pred_boxes, class_scores, class_inds, sub_list, obj_list, pred_list, predicate_inds, predicate_scores
    ...
    def evaluate(self, im_data, im_info, gt_objects, gt_relationships, gt_regions, 
        ...
        obj_boxes, obj_scores, obj_inds, subject_inds, object_inds, \
            subject_boxes, object_boxes, predicate_inds, predicate_scores = \
    ...
    def predict(self, im_data, im_info, gt_objects=None, gt_relationships=None, gt_regions=None,
        ...
        print ('Prediction time: ', time.time() - end)        
        cls_prob_object, bbox_object, object_rois = object_result[:3]
        cls_prob_predicate, mat_phrase = predicate_result[:2]

        class_pred_boxes, class_scores, class_inds, subject_inds, object_inds, \
            subject_boxes, object_boxes, predicate_inds, predicate_scores \
            = self.interpret_HDN(cls_prob_object, bbox_object, object_rois,
                                   cls_prob_predicate, mat_phrase, im_info,
                                   nms=nms, top_N=max(top_Ns), use_gt_boxes=use_gt_boxes)
        result = {
            ...
            'relationships': zip(subject_inds, object_inds, predicate_inds, predicate_scores),
            }
        return result
