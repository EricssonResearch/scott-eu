...
# Callback that syncs a string type topic and float type topic
def topic_callback(odom_data, scan_data):

    rospy.loginfo('odom_stamp: ' + str(odom_data.header.stamp.to_sec()) + 'scan_stamp: ' + str(scan_data.header.stamp.to_sec()))

    if(False):
        #Do something with string_data and float_data

        robot_pose = odom_data.pose
        robot_twist = odom_data.twist

        robot_scan = scan_data.ranges
...
#-- ROS SOAR NODE INITIALIZATION-----------------------------------    
    ...
    ts =  message_filters.ApproximateTimeSynchronizer([odom_sub, scan_sub], 10, 0.1)
