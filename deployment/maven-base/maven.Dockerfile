...
RUN echo "2019-05-10 23:48 CEST"
...
RUN git clone --depth=1 --branch=b96-logic https://github.com/eclipse/lyo.core.git && cd lyo.core/oslc4j-core-build && mvn clean install -Dmaven.test.skip=true -B -q
...
RUN git clone --depth=1 --branch=b-redesign https://github.com/eclipse/lyo.trs-server.git && cd lyo.trs-server && mvn clean install -Dmaven.test.skip=true -B -q
