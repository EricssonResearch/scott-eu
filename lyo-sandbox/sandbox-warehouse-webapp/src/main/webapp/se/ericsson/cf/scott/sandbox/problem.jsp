...
<%@page import="se.ericsson.cf.scott.sandbox.resources.Problem"%>
<%--
...
<%
  Problem aProblem = (Problem) request.getAttribute("aProblem");
%>
<%--
...
<head>
  ...
  <title><%= aProblem.toString(true) %></title>
...
<div class="container">
    <div class="page-header">
        <h1>Problem resource</h1>
        <p class="lead">URI:&nbsp;
      <a href="<%= aProblem.getAbout() %>"><%= aProblem.getAbout() %></a>
        </p>
    </div>
        <%--
...
End of user code
        ...
        <div>
          <dl class="row">
            <dt  class="col-sm-2 text-right">problemOutput</dt>
            <dd class="col-sm-9"><%= aProblem.problemOutputToHtml()%></dd>
          </dl>
        </div>
      </div>
    </div>
