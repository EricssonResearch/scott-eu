...
public class ResourceFactory {
  ...
  public WhObject buildObject(final String id, final String type, final Link place) {
    try {
      final WhObject object = new WhObject();
      object.setType(type);
      object.setIsOn(place);
      object.setAbout(WhObject.constructURI(basePath, spId, id));
      return object;
  ...
  public Place buildPlace(final String id, final String type, final Link waypoint) {
    try {
      ...
      place.setAbout(Place.constructURI(basePath, spId, id));
      place.setType(type);
  ...
  public Waypoint buildWaypoint(final String id, final Collection<Link> canMove) {
    try {
      ...
      waypoint.setAbout(Waypoint.constructURI(basePath, spId, id));
  ...
  public Link wayPointLink(final String id) {
    return new Link(Waypoint.constructURI(basePath, spId, id));
  }

  public Link placeLink(final String id) {
    return new Link(Place.constructURI(basePath, spId, id));
  }
