...
if clientID!=-1:
    ...
    if res==vrep.simx_return_ok:
        ...
        obj_list = ['ResizableFloor_5_25', 'stairs', 'slidingDoor', 'ConveyorBeltBody', 'Waypoint_CB', 'ShelfBody', 'TagShelf', 'DockStationBody', 'Bill', 'truck', 'product', 'slidingDoor', 'stairs']
    ...
    map_origin = obj_node_list['ResizableFloor_5_25'].bbox_min[0:1] 
    map_width = obj_node_list['ResizableFloor_5_25'].size[0]
    map_height = obj_node_list['ResizableFloor_5_25'].size[1]
    map_resolution = 0.20

    map_cells_x = map_width / map_resolution
    map_cells_y = map_height / map_resolution

    # PGM file header
    map_pgm = 'P2\n' + 'map_cells_x map_cells_y\n' + '255\n'
   
    # Create a empty matrix
    #map_grid = np.zeros((map_height, map_width))

    obj_node_list.remove('ResizableFloor_5_25')
    ...
    for obj in obj_node_list:
        obj_pos_list.append(box(obj.bbox_min[0], obj.bbox_min[1], obj.bbox_max[0], obj.bbox_max[1]))
    ...
    for i in range(map_cells_x):
        for j in range(map_cells_y):
            ...
            if obj_pol.intersects(Point(px,py)):
                map_pgm += '1 '
            else:
                map_pgm += '0 '
        map_pgm += '\n'
