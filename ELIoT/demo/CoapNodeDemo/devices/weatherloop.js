...
var cnode = new CoapNode('Weather_1', so);
...
function find(mode) {
  ...
  if (mode === '-t')  {
    ...
    console.log("Random latitude: " + latitude.toString());
    console.log("Random longitude: " + longitude.toString());
    weather.now ({lat:latitude, lon:longitude, APPID:'a7a67efb8526ba99aeb2b8f0d63cc18a'},function(err, json) {
        // APPID unique call for API
        if (!err) {
...
function RefreshFind(mode){
  ...
  RefreshStep = setInterval(find, 50000, "-t");
  }
